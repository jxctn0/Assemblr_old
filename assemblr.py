...
class CPU:
    ...
    def execute(self, opcode, operand, instructionNum):
        ...
        if opcode == 0x0:
            ...
            info("HLT", f"{line}Halt the program")
            return False
        elif opcode == 0x1:
            # SAV - Save the value in the accumulator to RAM[Address]
            address = operand
            info("SAV", f"{line}Save the value in the accumulator to RAM[{address}]")
            self.memory.write(address, self.acc)
        elif opcode == 0x2:
            # LDA - Load the value from RAM[Address] into the accumulator
            address = operand
            info("LDA", f"{line}Load the value from RAM[{address}] into the accumulator")
            self.acc = self.memory.read(address)
        elif opcode == 0x3:
            # ADD - Add the value to the accumulator
            value = operand
            info("ADD", f"{line}Add {value} to the accumulator")
            self.acc += self.memory.read(value)
        elif opcode == 0x4:
            # SUB - Subtract the value from the accumulator
            value = operand
            info("SUB", f"{line}Subtract {value} from the accumulator")
            self.acc -= self.memory.read(value)
        elif opcode == 0x5:
            # OUT - Output the value in the accumulator to the console
            info("OUT", f"{instructionNum} | OUT - Output the value in the accumulator to the console")
            print(f"Output: {self.acc}")
        elif opcode == 0x6:
            # INP - Input a value from the console
            info("INP", f"{instructionNum} | INP - Input a value from the console")
            self.acc = int(input("Enter a value: "))
        elif opcode == 0x7:
            # BAD - Bitwise AND the value with the accumulator
            value = operand
            info("BAD", f"{instructionNum} | BAD - Bitwise AND {value} with the accumulator")
            self.acc &= self.memory.read(value)
        elif opcode == 0x8:
            # BOR - Bitwise OR the value with the accumulator
            value = operand
            info("BOR", f"{instructionNum} | BOR - Bitwise OR {value} with the accumulator")
            self.acc |= self.memory.read(value)
        elif opcode == 0x9:
            # BXR - Bitwise XOR the value with the accumulator
            value = operand
            info("BXR", f"{instructionNum} | BXR - Bitwise XOR {value} with the accumulator")
            self.acc ^= self.memory.read(value)
        elif opcode == 0xA:
            # BNT - Bitwise NOT the accumulator
            info("BNT", f"{instructionNum} | BNT - Bitwise NOT the accumulator")
            self.acc = ~self.acc
        elif opcode == 0xB:
            # JMP - Jump Always to the specified label
            info("JMP",f"{instructionNum} | JMP - Jump Always to label {operand}")
            self.pc = operand
        elif opcode == 0xC:
            # JEZ - Jump to the specified label if the accumulator is equal to zero
            info("JEZ",f"{instructionNum} | JEZ - Jump to label {operand} if the accumulator is equal to zero")
            if self.acc == 0:
                self.pc = operand
        elif opcode == 0xD:
            # JGZ - Jump to the specified label if the accumulator is greater than zero
            info("JGZ",f"{instructionNum} | JGZ - Jump to label {operand} if the accumulator is greater than zero")
            if self.acc > 0:
                self.pc = operand
        elif opcode == 0xE:
            # JLZ - Jump to the specified label if the accumulator is less than zero
            info("JLZ", f"{instructionNum} | JLZ - Jump to label {operand} if the accumulator is less than zero")
            if self.acc < 0:
                self.pc = operand
        elif opcode == 0xF:
            # SIG - Send a signal to the specified pin (ONLY FOR RASPBERRY PI/MICR0PYTHON)
            info("SIG", f"{line}Send a signal to pin {operand} (ONLY FOR RASPBERRY PI/MICR0PYTHON)")
            pass
        return True
